...
if($row){
	$redis = new Redis();
	//$redis->connect('192.168.1.158', 6379);内网测试
	//$redis->connect('101.200.187.142', 6379);外网测试
	$redis->connect('10.51.58.70', 6379);
	$redis->auth('fNENPzes6Ndk5nElWFpSMAF1zE0bMb');
	$key = "fypadmin.szprize.cn#_Feed_";
	//$feed_callback = 'http://192.168.1.158:8083/';内网测试
	//$feed_callback = 'http://launchertest.szprize.cn';外网测试
	$feed_callback = 'http://launcher.szprize.cn/';
	foreach($row as $v){
